...
/**
 * connection to the blockchain.info API for getting bitcoin transactions from the
 * blockchain.
 *
 *
 * @constructor
...
function BlockchainAPIConnection() {
    ...
    // always be pinging to keep the websocket alive
    ...
    function connect() {
        websocket = new WebSocket('ws://ws.blockchain.info/inv');
        ...
        websocket.on('close', function() {
            console.log("blockchain.info api: closed");
            connect();
        });
    ...
    /**
     * notify any listeners that an update to the given address has happened
     * @param address public address
     */
    ...
    /**
     * subscribes to the given addresses on the websocket
     */
    ...
    function findWallet(address) {
        for(var i=0; i<wallets.length; i++) {
            if(wallets[i].addresses.indexOf(address)>=0)
                return wallets[i];
        }
        return null;
    }
    ...
    return {
        ...
        refund: function(address) {
            var wallet = findWallet(address);
            if(!wallet) throw "cannot find a wallet with that address!";
            var balance = wallet.getBalance();

            if(balance == 0) {
                console.log("can't refund an empty wallet");
                return;
            }
        ...
        /**
         * registers a callback to be called whenever the wallet/invoice behind the given address is updated
         * @param address address to wallet
